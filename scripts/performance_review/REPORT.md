...
### Expanded modulation panels: paused CPU and repaint lifecycle

The user clarified that UI performance comparisons must expose every modulation graph. The following measurements explicitly expand all four modulation panels, enable MIDI so the envelope is visible, and verify nonzero graph bounds from full accessibility snapshots. The fixed window is 1,500×900, each panel is 263 pixels high, the visualiser is paused, no notes are held, and audio input/output remain disabled/muted in isolated profiles. Initial collapsed-panel observations were excluded from these comparisons. Background windows and programmatic resizing do not measure native live-resize presentation.

Source tracing found that tab values requested repaint every 60 Hz tick even when no active assignment made the meter visible. The setter now keeps consuming values and smoothing but requests that meter repaint only when it is displayed; activation, assignment changes and hover retain their existing redraw paths. Envelope/LFO trail clearing also repeatedly requested redraws and could keep an allocated-but-empty trail's timer running indefinitely. Clearing is now idempotent, allocated storage is distinguished from active trail data, and reset stops the timer while always clearing old bridge history. Keeping that history reset is necessary even after natural expiry, to avoid a spurious connecting line when animation restarts. Active-marker updates and fading remain unchanged. A width-one trail remap also divided by zero; its denominator is now bounded.

Thirty production-header lifecycle/width checks passed under ASan, UBSan and float-cast-overflow instrumentation, covering repeated clear/reset, retained fading, expiry, restart and active widths 32→1→0→1→20→1→300→1. The helper links the existing product library; the tested header is instrumented, not the entire linked application. Leak detection was disabled for this helper. Local sources/results are `paused-ui-followup/trail_probe.cpp`, `build_trail_probe.py` and `trail-results.log`.

Three alternating before/after pairs each measured ten seconds of idle process CPU and six seconds of paced resize requests at a target 60 Hz. Median CPU was approximately 1.126→1.098 cores, with overlapping ranges of 1.078–1.127 and 1.085–1.105. Median-run resize p50 was 59.72→61.09 ms and p99 114.06→111.80 ms. These results do not establish a meaningful overall CPU or resize improvement. The small changes are retained for avoiding objectively redundant requests and fixing trail lifecycle/width correctness; they are not presented as a solution to expanded-panel rendering cost. Measurements include the audio callback and other process threads; no compiler or profiler ran during timed intervals, although the user continued using the laptop.

Pausing the visualiser does not pause modulation. The LFO can continue advancing, sidechain display samples keep reporting active even at a stationary silent value, and random history continues appending samples. Those paths still repaint the graphs. The sampled stacks show substantial native Core Animation/JUCE painting; removing some idle requests need not change the overall redraw cadence while other graphs keep updating. Disabling these animations merely because the visualiser is paused would change UI behavior and was not introduced. Avoiding stationary-history painting would need to preserve old-trail decay and scrolling history, not just compare the latest scalar value.

A separate baseline/alternative/alternative/baseline comparison reused the preserved multiple-paint-calls app with the same expanded layout. Its idle process CPU was 0.648–0.660 cores versus 1.102–1.129 for the normal path; resize p50 was 20.23–20.61 ms versus 56.86–57.39 ms. This comparison app predates the latest small fixes and Tahoe presentation patch, so it is supporting evidence rather than a same-revision production acceptance test. The user's earlier native drag/resize comparison felt unchanged. The painting flag remains unshipped; a combined current-build/native presentation comparison is required before adoption. Local profiles and snapshots remain under `paused-ui-followup/`; raw user settings and traces are not published.

Final osci-render Profiling standalone and VST3 builds passed with the trail/tab changes. The standalone passed resize and Audio/Recording/About open-dismiss checks, then verified all four modulation panels expanded at 263 pixels high. Sosci does not compile the changed modulation components; its preceding final build/smoke remains applicable. No new audio processing or rendering-semaphore change was introduced.


### Current-build painting-path comparison

A subsequent comparison rebuilt the current source, including the accepted Tahoe buffer-swap patch and trail fixes, with only `JUCE_COREGRAPHICS_RENDER_WITH_MULTIPLE_PAINT_CALLS=1` added to its temporary Profiling configuration. Normal project/generated-header bytes were restored afterward; the comparison app uses a separate output directory. Three alternating pairs used the same expanded four-panel, paused, MIDI-enabled/no-held-note profile and 1,500×900 window as above. Median process CPU was 1.093→0.524 cores (approximately 52% lower), with ranges 1.084–1.109 and 0.522–0.526. Median-run resize p50 was 67.01→24.85 ms and p99 115.40→43.54 ms. These remain background/on-another-Space measurements, not visible-frame or native live-resize validation. The normal app window was explicitly observed offscreen in the window-server list; accessibility visibility verifies expanded layout, not physical onscreen presentation.

JUCE's [documented tradeoff](https://github.com/juce-framework/JUCE/blob/9.0.1/BREAKING_CHANGES.md) and [maintainer explanation](https://forum.juce.com/t/direct2d-juce-coregraphics-render-with-multiple-paint-calls/59264/2) match the observed mechanism: separate dirty regions can avoid redrawing large intervening areas, but the Metal-backed path gives up default asynchronous CoreGraphics rendering and can regress other workloads. Local native-source review found no direct incompatibility with the separate OpenGL swap fix. Transparent peers still have alpha support and external drag windows remain separate peers above GL.

Native live resize follows a different synchronous Metal presentation path (`nextDrawable`, `waitUntilScheduled`, `present`) and recreates drawing resources when the drawable size changes. Both painting variants also retain JUCE's approximately 30 Hz deferred-repaint throttle during native live resize; its source comment cites event-queue responsiveness and Logic parameter recording. That throttle was not changed. The measured background improvement therefore does not contradict the user's earlier report of unchanged native resize feel.

The current comparison is retained at `paused-ui-followup/current-flag1/app/osci-render.app`, with exact build/restoration records alongside it. The flag remains disabled in production pending visible drag-over-GL, transparent popout/native resize and editor lifecycle checks. The next user interaction test should use this combined current build, not the older flag-only app.


## Follow-up: packed line rendering with CPU Lanczos

Stage 1 from `GPU_RENDERER_FOLLOWUP.md` is implemented without upgrading the OpenGL context. Supported contexts upload each point once and draw one instance per adjacent segment. Position, brightness and optional RGB share one reusable CPU vector and one upload; the separate colour buffer is removed. Unsupported contexts retain the previous duplicated draw through the same packing code. CPU filtering, fragment shaders, triangle order, shutter arithmetic and the render semaphore/handoff are unchanged. The runtime change is about 40 net lines across three renderer files.

The final production shader's instanced and fallback branches both matched the historical renderer in 504 cases: 264,241,152 HDR scalar values and 125,338,752 geometry/varying values were bit-identical. Cases include XY/XYZ/RGB, short brightness input, negative-red sentinel, zero-length segments, sparse/dense paths, shutter, transparency, fisheye and aspect changes. A following fullscreen draw also matched after attribute-divisor restoration. The extracted production packing loop passed 36,000 bit comparisons under ASan/UBSan, including signed zero, infinities and NaN payloads; those nonfinite cases were CPU checks, not HDR rendering fixtures. These are M3 Pro/legacy OpenGL results, not proof for every driver.

At 4,800 output points, uploaded bytes fall from 230,400 to 57,600 for XY/XYZ and 460,800 to 115,200 for RGB (75%). In six balanced order rotations of an isolated 256-square HDR line probe, RGB packing fell 23.09→6.86 µs and upload submission 6.00→1.79 µs. Completed pack/upload/clear/line-draw time fell 98.58→81.57 µs for the smooth fixture and 575.16→550.37 µs for dense crossings. XY smooth fell 79.99→74.58 µs. At 19,200 RGB points, smooth fell 447.30→288.43 µs. These completed-stage timings include a batched GPU finish, exclude blur/composition/presentation, and contain no readback. The fallback retained its upload size; its XY packing was 7.49→10.66 µs, with completed stage approximately unchanged (79.99→80.80 µs).

The application comparison used matching normal-painting-path Profiling builds, isolated muted/input-disabled profiles, 96 kHz/512-sample device settings, a 1,024-square Empty canvas, and five alternating pairs with upsampling on and off. Each run recorded 600 fresh-audio frames after 60 warmup frames: 12,000 measured frames total. Every candidate capture explicitly reports `instanced_lines: true` on OpenGL 2.1 Metal; baseline reports false. No queries were dropped or pending at export. The stage queries included 95 zero-duration results, which are retained rather than interpreted as free work.

Median across run statistics, in microseconds:

| Upsampling | CPU scope | Previous median / p99 | Packed median / p99 |
| --- | --- | ---: | ---: |
| Off | Complete instrumented GL render scope | 1212.88 / 2988.08 | 1110.58 / 3042.92 |
| Off | Line stage | 290.75 / 1138.38 | 263.00 / 1052.96 |
| On | Complete instrumented GL render scope | 1220.54 / 3118.00 | 1130.62 / 3020.29 |
| On | Line stage | 389.62 / 1315.75 | 304.54 / 1144.25 |

CPU render-scope medians improved in all ten pairs; median-of-run medians fell about 7–8%, and upsampled line-stage CPU time about 22%. Off-mode render p99 rose about 1.8%, while on-mode p99 fell about 3.1%; tail improvements are not universal. GPU line elapsed medians were 475.17→531.38 µs off and 625.25→619.12 µs on, so this does not establish a GPU speedup. Query scopes include driver/scheduling effects and change batching; do not add CPU and GPU times. The CPU render scope excludes the separate Lanczos worker and native presentation completion. These background application captures do not measure displayed FPS, energy or whole-application CPU usage.

The change is retained for the repeatable CPU reduction and smaller uploads with exact output, without introducing a GPU filtering backend. Local evidence is in `build/performance-review/packed-lines/`: `EVIDENCE.md`, `final-timings.csv`, `final-timing-summary.json`, `application-summary.json`, `capture-manifest.jsonl`, and exact-output/sanitizer logs. A faulty intermediate harness timing run is explicitly rejected; an application batch potentially overlapping a probe was excluded in full and replaced by the clean five-pair run. Temporary instrumentation was restored byte-for-byte before clean builds.

Clean final builds passed for osci-render Profiling standalone/VST3 and sosci Debug standalone. The normal osci-render binary passed isolated launch/visualiser-tree and five resize checks; sosci passed isolated resize plus Audio/Recording/About open-dismiss checks. Test copies were muted with input disabled and closed afterward. Final source hashes match the exact candidate validated before instrumentation; generated headers were restored for osci-render. Windows/Linux hardware rendering and native displayed-FPS checks were not rerun for this stage.


## Native resizing follow-up with the user

The user repeated dragging/resizing with all four modulation panels expanded and reported that the preserved alternative painting build remained poor, about the same as the normal path. The alternative flag stays disabled. The first System Trace request lasted 60 seconds but retained 10 seconds; statements below refer to the retained window, not the requested duration. Filtering samples to native resize stacks attributed 81.83% of main-thread CPU to component painting, including 14.23% to FlowTrailRenderer::paintTrail. Inclusive percentages overlap. No potential-hangs event over 250ms was recorded; this is not evidence of smooth frame pacing.

A small retained fix now skips paintTrail entirely when it has no data, and skips the stretched image draw when every computed alpha is zero. Existing alpha/glow calculations and active trails are unchanged. Previously an allocated but cleared/faded trail could still perform a full clipped image draw on every resize. The before/after probe compared 3600 cases,70601400 pixels and glow values exactly, including custom fade times, reset/expiry, widths 1/177, heights 1/200 and scales 1/1.25/2. Isolated native 640×300@2× empty painting fell approximately 3237 µs→nearzero; fully faded painting 3225 µs→3.34 µs; active painting remained approximately 3306 µs. These are trail-only measurements. Profiling standalone and VST3 builds passed. The user described the subsequent native resize check as slightly better, still short of smooth resizing.

The second System Trace explicitly retained 45.067 seconds. All main-thread sampled CPU totals 14.847 seconds;13.231 seconds of samples contain native resize. Within those native samples, painting accounts for 37.69% and JUCE OpenGL handleResize for 10.98%. Main-thread nonidle syscall waiting across the capture totals 25.456 seconds, including 10.934 seconds of OpenGL presentation waits and at least 8.572 seconds of CoreAnimation backing-store queue waits. Native GL surface updates contribute approximately 2.976 seconds. These distributions come from different painting backends and different manual gestures, so they are diagnostic attribution, not a paired percentage speedup for the trail fix. Many shorter waits remain even though no half-second pause recurred.

Further local experiments were not adopted: clipping active trail image drawing changed pixels; replacing sampled straight segments with exact endpoints changed edge antialiasing but gave small/mixed paint-only gains (flat 445→425 µs, slope 620→607 µs, DAHDSR 618→575 µs, mixed curved/flat 612→622 µs). Drawing grids directly saved approximately 75 µs for a changing-size uniform grid, but labelled grids regressed during static redraws and rounded clipping changed a few boundary pixels. These did not justify claiming a substantial resize improvement.

JUCE's synchronous macOS OpenGL resize rendering has an explicit correctness purpose: keeping the drawable current rather than presenting a stretched old buffer. No application rendering semaphore or synchronous resize code was changed. A separate temporary swap-interval experiment targets the measured presentation waits; it is not yet an accepted runtime change. Local evidence: `build/performance-review/paused-ui-followup/user-comparison/` (`TRACE_FINDINGS.md`, `trail-candidate/main-wait-analysis.json`, `trail-skip/`, `straight-path/`, `grid-direct/`).
...
## Live-resize snapshot experiments (rejected user experience)

A local Profiling comparison temporarily installed a custom JUCE CachedComponentImage on visible NodeGraph/RandomGraph components during native window live resize. It captured graph/handle pixels at display resolution, scaled them during resizing, suppressed same-bounds animation repaint requests, and restored fresh drawing on resize end. Bounds-change invalidation explicitly repainted both old and new regions. Exact-helper hidden-peer tests passed lifecycle, handles, mutation freeze/resume, existing/replaced cache ownership, deletion, released-image fallback and expanded-region invalidation. Initial cached/direct rendering differed at46 of1,920,000 pixels; frozen state itself stayed exact through mutations. No production graph code changed.

The cache was not a performance win in the offscreen2x fixture: median repeat-mean Node resize/paint1952.96→2247.62us, Random2074.06→2230.08us. Initial capture medians1.77/1.92ms. An isolated application diagnostic confirmed four graphs captured/restored, with13.804ms total capture cost; programmatic geometry timing did not show a benefit. The user then tried the native comparison and rejected the frozen/stretching appearance as visually distracting, preferring live animation. The normal muted application was restored. A separate native compositor graph snapshot prototype was prepared but not built or tested after that feedback. Neither snapshot approach is retained.

An independent simple OpenGL reproduction tested a synthetic frozen scene in a compositor layer while hiding the native GL view and keeping its geometry fixed. In one instrumented programmatic sequence, child layout median7.735→0.048ms, whole-window size-change median11.110→3.624ms, and combined size/movement11.832→3.305ms. Native child frame mutations fell38→4 and context updates152→86; timer medians remained about20ms. Capture and restore each occurred twice, at medians3.184ms and9.059ms respectively. This is mechanism evidence, not native live-resize FPS, a repeated performance result, or actual visualiser output validation. It motivates removing native GL surface resizing while preserving live frames, not adopting freezing.
