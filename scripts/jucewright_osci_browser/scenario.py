...
class OsciRenderBrowserRun(ControlDiscoveryMixin, BrowserSession):
    ...
    def exercise_recording_dialog(self) -> None:
        ...
        self.try_step("set recording resolution landscape", self.cli("select-option", "--role", "comboBox", "--name", "Resolution", "--exact", "--text", "1920 x 1080", "--timeout-ms", "3000"))
    ...
    def configure_visualiser_recording_for_automation(self) -> None:
        ...
        self.try_step("recording resolution custom", self.cli("select-option", "--role", "comboBox", "--name", "Resolution", "--exact", "--text", "Custom", "--timeout-ms", "3000"))
        self.try_step("recording canvas width low", self.cli("set-value", "--role", "slider", "--name", "Canvas Width", "--exact", "--timeout-ms", "3000", "256"))
        self.try_step("recording canvas height low", self.cli("set-value", "--role", "slider", "--name", "Canvas Height", "--exact", "--timeout-ms", "3000", "256"))
    ...
    def prepare_external_fixtures(self, external_types: list[str]) -> None:
        self.fixture_dir.mkdir(parents=True, exist_ok=True)
        fixtures = {
            "gpla": self.root_dir / "Resources" / "gpla" / "fallback.gpla",
            "gif": self.root_dir / "images" / "demo1.gif",
        ...
        for kind in external_types:
            ...
            if source is None:
                self.die(f"No fixture source configured for external file type '{kind}'")
            shutil.copyfile(source, self.fixture_dir / f"automation.{kind}")
    ...
    def exercise_external_file_type_passes(self) -> None:
        ...
        self.prepare_external_fixtures(external_types)
    ...
    def run(self) -> int:
        try:
            ...
            self.try_step("visualiser screenshot nonblank check before file is loaded", lambda: self.check_visualiser_png_not_blank(self.artifact_dir / "visualiser.png"))
