...
OsciLottieParser::OsciLottieParser(juce::String jsonContent, std::function<void(juce::String)> errorCallback)
...
std::vector<std::unique_ptr<osci::Shape>> OsciLottieParser::draw() {
    ...
    const int frameCount = totalFrames.load(std::memory_order_relaxed);
    if (frame < 0 || frame >= frameCount) return out;
    const auto& src = getOrCreateCachedFrame(frame);
...
const std::vector<std::unique_ptr<osci::Shape>>& OsciLottieParser::getOrCreateCachedFrame(int index) {
    for (const auto& cachedFrame : framesCache) {
        if (cachedFrame.index == index) {
            return cachedFrame.shapes;
        }
    }

    CachedFrame cachedFrame;
    cachedFrame.index = index;
    animation->frame((float)index);
    extractShapesAtCurrentFrame(cachedFrame.shapes);

    if (framesCache.size() >= maxCachedFrames) {
        framesCache.pop_front();
    }
    framesCache.push_back(std::move(cachedFrame));
    return framesCache.back().shapes;
}
...
void OsciLottieParser::showError(juce::String message) {
    if (errorCallback != nullptr) {
        errorCallback(std::move(message));
    } else {
        juce::Logger::writeToLog("Lottie: " + message);
    }
}
...
void OsciLottieParser::fallbackShapes() {
    ...
    fallbackFrame.shapes.push_back(std::make_unique<osci::Line>(-0.5, -0.5, 0.5, 0.5));
    fallbackFrame.shapes.push_back(std::make_unique<osci::Line>(-0.5, 0.5, 0.5, -0.5));
    framesCache.push_back(std::move(fallbackFrame));
